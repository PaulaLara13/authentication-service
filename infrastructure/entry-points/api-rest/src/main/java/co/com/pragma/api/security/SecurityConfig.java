    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        return http
            ...
            .authorizeExchange(exchanges -> exchanges
                ...
                .pathMatchers("/swagger-ui.html", "/swagger-resources/**").permitAll()
                .pathMatchers("/favicon.ico").permitAll()
                .pathMatchers("/actuator/health").permitAll()
                .pathMatchers(HttpMethod.OPTIONS).permitAll()
                // Reglas por rol
                .pathMatchers(HttpMethod.POST, "/api/v1/user/**").hasAnyRole("ADMIN", "ADVISOR")
		            .pathMatchers(HttpMethod.DELETE, "/api/v1/user/**").hasAnyRole("ADMIN", "ADVISOR")
                // El resto requiere autenticación
                .anyExchange().authenticated()
            )
            .headers(headers -> headers.frameOptions(frame -> frame.mode(XFrameOptionsServerHttpHeadersWriter.Mode.SAMEORIGIN))) // H2 Console
